...
age_check as (
  select
    *,
    case
      when age is not null and age_cod is null and (age_grp is null or age_grp is not null) then True
...
get_origin_country as (
  select 
    *,
    case 
      when reporter_country = 'COUNTRY NOT SPECIFIED' and occr_country is not null then occr_country
      when reporter_country <> 'COUNTRY NOT SPECIFIED' and occr_country is not null then occr_country
      when reporter_country = 'COUNTRY NOT SPECIFIED' and occr_country is null then 'COUNTRY NOT SPECIFIED'
      when reporter_country <> 'COUNTRY NOT SPECIFIED' and occr_country is null then reporter_country
...
conv_lbs_to_kg as (
  select 
    *,
    ...
    case 
      when wt is not null and wt_cod is null then True else False
    end wt_check
  ...
  select 
    *,
    case 
      ...
      when age_bin = 'Neonate' and (wt_kg >= 2.5 and wt_kg < 4.5) then "Valid Weight"
      when age_bin = 'Infant' and (wt_kg >= 4.4 and wt_kg < 11.3) then "Valid Weight"
      when age_bin = 'Toddler' and (wt_kg >= 8.5 and wt_kg < 17.5) then "Valid Weight"
      when age_bin = 'Preschooler' and (wt_kg >= 12.5 and wt_kg < 25.8) then "Valid Weight"
      when age_bin = 'Child' and (wt_kg >= 19.8 and wt_kg < 51) then "Valid Weight"
      when age_bin = 'Teenager' and (wt_kg >= 39 and wt_kg < 81) then "Valid Weight" 
      else "Invalid Weight"
...
final as (
  ...
  where outlier_check <> 'Invalid Weight'
  and age_yrs > 0
