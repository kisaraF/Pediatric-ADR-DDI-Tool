...
def removeZeroAge(df):
   df = df[df['age_yrs'] != 0]
   return df

#Step 20: Only get the necessary columns & select new cases only (for post-covid data)
def finalLayer(df):
   df = df[['primaryid', 'caseid', 'i_f_code', 'age_yrs', 'age_bin', 'sex', 'wt_kg', 'Origin_country', 'init_fda_dt', 'fda_dt']]
   df = df[df['init_fda_dt'] >= 20210101]
   return df

#Step 21: Export as CSV
